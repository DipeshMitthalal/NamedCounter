...
public class Welcome {
	...
	public Response getValue(@PathParam("param") String namedParameter) {
		...
		if (output.equals("NA"))
			return Response.status(Status.BAD_REQUEST).entity("counter with name:" + namedParameter + "does not exist")
					.build();
		else
			return Response.status(Status.ACCEPTED).entity(namedParameter + ":" + output).build();
	}

	/**
	 * Increments the value of the named counter and return the name of the
	 * named counter and its value. Name of the namedcounter is passed as an
	 * argument
	 * 
	 * @param namedParameter
	 *            name of counter
	...
	public Response incrementValue(@PathParam("param") String namedParameter) {
		String output = SingletonClass.getInstance().incrementValueInMap(namedParameter);
		if (output.equals("NA"))
			return Response.status(Status.BAD_REQUEST).entity("counter with name:" + namedParameter + "does not exist")
					.build();
		else
	...
	/**
	 * This will create a named counter, initialize it to 0 and put it in
	 * Hashmap. In Java one cannot create a variable with dynamic name
	 * 
	 * @param namedParamter
	 *            name of the namedcounter
	...
	public Response createNamedParamter(@PathParam("param1") String namedParamter) {
		...
		return Response.status(Status.ACCEPTED)
				.entity(SingletonClass.getInstance().addNamedCounter(namedParamter) + ":" + 0).build();
	}
	...
	public Response help() {
		String response = "<html> " + "<body>" + "<p>Welcome to Named Counter Assignment</p>" + "<p>Usage:</p>"
